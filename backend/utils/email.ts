...
// Helper to send email via Resend API (HTTPS Port 443 - Firewall Friendly)
const sendViaResend = (to: string, subject: string, html: string, text?: string): Promise<boolean> => {
    return new Promise((resolve) => {
        ...
        const data = JSON.stringify({
            from: 'Secure Wallet <onboarding@resend.dev>', // Default for testing
            to: [to],
            subject: subject,
            html: html,
            text: text || html.replace(/<[^>]*>?/gm, '') // Strip tags for text version
        });
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                if (res.statusCode === 200 || res.statusCode === 201) {
                    console.log(`[Resend] Email sent to ${to}. ID: ${JSON.parse(body).id}`);
                    resolve(true);
                } else {
                    console.error(`[Resend] Failed to send to ${to}. Status: ${res.statusCode}. Response: ${body}`);
                    resolve(false);
                }
            });
        });
...
// Re-export for debug route to use generic send
export const sendDebugEmail = sendViaResend;
